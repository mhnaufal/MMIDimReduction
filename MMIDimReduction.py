...
class MMINet:
    ...
    def _instantaneousLoss(self, proj_array, input_labels, ind):
        ...
        obs_labels = [
            np.where(np.delete(input_labels, ind) == c)[0] for c in range(num_classes)
        ]
        ...
        joint_prob = [
            priors[c]
            * constants[c]
            * F.exp(F.logsumexp(energies[c]) - F.logsumexp(energies[max_index]))
            for c in range(num_classes)
        ]
        ...
        for c in range(num_classes):
            ...
            conditional_entropy_parts.append(
                c_given_y * (F.log(constants[c]) + F.logsumexp(energies[c]))
            )
            entropy_parts.append(
                priors[c] * constants[c] * F.exp(F.logsumexp(energies[c]))
            )
    ...
    def _kdeparts(self, input_obs, input_ins):
        ...
        silverman_factor = np.power(n * (d + 2.0) / 4.0, -1.0 / (d + 4))
        ...
        energy = -0.5 * F.diagonal(
            F.matmul(F.matmul(diff, F.inv(kernel_bw)), F.transpose(diff))
        )
...
class MLP(chainer.Chain):
    def __init__(self, input_dim, output_dim):
        ...
        super(MLP, self).__init__(
            l1=L.Linear(
                input_dim, int(input_dim // 2), initialW=LeCunUniform(), nobias=True
            ),
            l2=L.Linear(
                int(input_dim // 2),
                int(input_dim // 4),
                initialW=LeCunUniform(),
                nobias=True,
            ),
            l3=L.Linear(
                int(input_dim // 4), output_dim, initialW=LeCunUniform(), nobias=True
            ),
        )
